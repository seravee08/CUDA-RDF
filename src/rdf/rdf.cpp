...
namespace rdf{
	...
	void RDF::trainNodesRecurse(std::vector<rdf::Node>& nodes,
		...
		// =================== Test 1 =======================	
		//if (recurseDepth >= 1) {
		//	std::cout << idx_dataS << "  " << idx_dataE << "(" << idx_node << ")" << endl;
		//}
		...
		for (int f = 0; f < numOfCandidateFeatures_; f++){
			...
			//for (int kk = 0; kk < nThresholds; kk++)
			//	cout << thresholds[kk] << endl;
			//system("pause");
		...
		//cout << "Depth: " << recurseDepth + 1 << "  Left:  " << idx_dataS << "   " << idx << endl;
		//cout << "Depth: " << recurseDepth + 1 << "  Right:  " << idx << "   " << idx_dataE << endl;
		...
		//cout << recurseDepth << endl;
	}


	rdf::Tree RDF::trainTree(const int& idx, const int& maxDepth, RDF_CU& rdf_cu){
		// ============================================
		// This is the entrance to the CUDA code
		// The parallelism across trees is to be achieved using streams (TODO)
		// ============================================
		...
		// ===== CUDA Call =====
		//RDF_CU rdf_cu;
		//cu_params_copy((*samples_)[idx], rdf_cu);
		//rdf_cu.compute_responses();
		...
		//cu_data_copy((*samples_)[idx], rdf_cu);
